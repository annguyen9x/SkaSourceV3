...
@WebServlet(name = "GiaoHangCapNhatTaiKhoan", urlPatterns = { "/GiaoHangCapNhatTaiKhoan" })
public class GiaoHangCapNhatTaiKhoanController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( tenNV.length() >= 2 && matKhau.length() >= 6 && email.length() > 0 && dienThoai.length() >= 10 && !gioiTinh.equals("") 
			...
			writer.print("location='GiaoHangCapNhatTaiKhoan'");
			writer.print("</script>");
		}
	}
